...
public class GenEntityMySQL {
	...
	// "jdbc:oracle:thin:@192.168.215.128:1521:orcl";
	...
	public void init() {
		...
		try {
			...
			String comSql = "select column_comment from information_schema.COLUMNS where TABLE_SCHEMA='"
					+ databaseName
					+ "' and  table_name='"
					+ tablename.toUpperCase() + "'";
			// String comSql =
			// "select * from user_col_comments where  table_name='"+tablename.toUpperCase()+"'";
			...
			for (int i = 0; i < colnames.length; i++) {
				...
				for (int k = 0; k < strs.length; k++) {
					...
					if (k == 0) {
						...
						colnames[i] += num.substring(0, 1).toUpperCase()
								+ num.substring(1);
					}
				}
			}
